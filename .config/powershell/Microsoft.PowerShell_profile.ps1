...
Set-Alias -Name ff -Value "$env:LOCALAPPDATA\Mozilla Firefox\firefox.exe"
...
function Start-Emacs($File) {
    ...
    $params = "-f $env:APPDATA\.emacs.d\server\server $File"
...
function Write-FileHash {
    Get-ChildItem | ForEach-Object {(Get-FileHash $_.name).hash + " *" + $_.name | out-file ($_.name + ".sha256")}
}
